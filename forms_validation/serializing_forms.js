document.addEventListener('DOMContentLoaded', () => {
  ...
  function parseFormData(form) {
    ...
    let formData = formElements.reduce((formData, currentElement) => {
      if (currentElement.tagName !== 'INPUT' || currentElement.name === 'creditCard') return formData;

      formData += `${encodeURIComponent(currentElement.name)}=${encodeURIComponent(currentElement.value)}&`;

      return formData;
    }, '');

    formData += `creditCard=${encodeURIComponent(getCreditCardNumber())}`;
  ...
  form.addEventListener('submit', event => {
    ...
    if (form.checkValidity()) {
      ...
      document.querySelector('#serializedFormData').textContent = formData;
    } else {
